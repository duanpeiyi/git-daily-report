...
```bash
bash setup-launchd.sh          # 安装/更新（日报每天 18:30 + 周报每周五 18:30）
bash setup-launchd.sh remove   # 卸载

launchctl list | grep git-report    # 查看是否已加载
cat run.log                         # 查看运行日志
```

改时间：编辑 `setup-launchd.sh` 里的 `Hour` / `Minute`（周报还有 `Weekday`，1=周一…5=周五），改完重新执行 `bash setup-launchd.sh`。
